...
export function TutorsCard({ tutors, selectedCategory }: any) {
  if (!tutors || tutors.length === 0) {
    return (
      <div className=" flex flex-col items-center text-center animate-fadeIn">
        <div className="w-20 h-20 rounded-full bg-gradient-to-r from-[#5624D0] to-[#b00ea5] 
        ...
        <Link
          href="/"
          className="mt-5 inline-block px-6 py-2 rounded-full text-white font-semibold
            bg-gradient-to-r from-[#5624D0] to-[#b00ea5] shadow-md hover:shadow-lg
  ...
  return (
    <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 ">
      {tutors?.map((tutor: any) => (
        <div
          key={tutor.id}
          className="px-5 py-3 rounded-xl border border-gray-200 shadow-sm hover:shadow-lg 
...
/>
          ...
          <div className="">
            ...
            <div className="mt-3 flex flex-wrap gap-1.5 px-2">
              {tutor.subject?.map((sub: string) => (
                <span
                  key={sub}
                  className="px-2 py-0.5 bg-[#f3e8ff] text-[#5624D0] 
                  font-medium rounded-full text-[10px] shadow-xs"
                >
                  {sub}
                </span>
              ))}
            ...
            <Link
              ...
              className="block w-full text-center py-2 mt-5 rounded-md font-semibold 
                ...
                shadow hover:opacity-90 transition text-center"
            >
